...
@section('content')
    <div class="content">
        <form @if(! $view_model->getModelData('id') ) action="{{ route('admin.blog.post-category.store') }}" method="POST"
              @else action="{{ route('admin.blog.post-category.update' , $view_model->getModelData('id') ) }}"
