...
@section('content')
    <div class="content">
        <form @if(! $view_model->getModelData('id') ) action="{{ route('admin.shop.brand.store') }}" method="POST"
              @else action="{{ route('admin.shop.brand.update' , $view_model->getModelData('id') ) }}"
