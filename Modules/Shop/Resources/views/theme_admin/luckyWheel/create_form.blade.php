...
@section('content')
    ...
    <div class="content">
        ...
        <div class="row">
            <form action="{{ route('admin.shop.luckyWheel.store') }}" method="POST">
                @csrf
                <div class="mb-4 col-6">
                    <label for="title" class="block text-gray-700 text-sm font-bold mb-2">عنوان:</label>
                    <input type="text" name="title" id="title" class="w-full px-3 py-2 border border-gray-300 rounded" value="{{ old('title') }}"
                           required>
                    @error('title')
                ...
                <div class="mb-4 col-6">
                    ...
                    <select name="product_catalog_id" id="product_id" class="w-full " required>
                        <option class="px-5 mx-5" selected disabled> انتخاب محصول</option>
                        @foreach($products as $product)
                            <option class="px-5 mx-5 " value="{{ old('product_catalog_id',$product->id) }}">{{ $product->title }}</option>
                        @endforeach
                    </select>
                    @error('product_catalog_id')
                    <div class="alert-danger-soft mt-2">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-4 col-6">
                    ...
                    <select name="products[]" id="products" class="w-full " multiple >
                        @foreach($products as $product)
                            <option class="px-5 mx-5" value="{{ $product->id }}">{{ $product->title }}</option>
                        @endforeach
                    </select>
                    @error('product_catalog_id.*')
                    <div class="text-red-500 mt-2">{{ $message }}</div>
                ...
                <div class="mb-4 col-6">
                    ...
                    <select name="discount_code[]" id="discount_code" class="w-full px-3 py-2 border border-gray-300 rounded" multiple >
                        @foreach($copans as $copan)
                            <option value="{{ $copan->id }}">{{ $copan->code }}</option>
                        @endforeach
                    </select>
                    @error('discount_code.*')
                    <div class="text-red-500 mt-2">{{ $message }}</div>
