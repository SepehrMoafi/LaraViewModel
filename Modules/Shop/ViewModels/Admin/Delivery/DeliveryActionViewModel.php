...
class DeliveryActionViewModel extends BaseViewModel
{
    ...
    {
        $this->modelData = new DeliveryType();
        $data = [
            'view_model' => $this,
        ];
        return $this->renderView('shop::Delivery.form' ,$data);
    }
    ...
    {
        $this->modelData = DeliveryType::find( $request->model_id );
        $data=[
            'view_model' => $this,
        ];
        return $this->renderView('shop::Delivery.form' ,$data);
    ...
    public function destroy($request)
    {
        try {
            ...
            $model = DeliveryType::find( $request->model_id );
            $model->delete();
            alert()->success('با موفقیت انجام شد');
            DB::commit();
            return redirect(route('admin.shop.delivery.index'));
    ...
    public function saveService( $validData ,  $request)
    {
        try {
            DB::beginTransaction();
            $model = DeliveryType::find( $request->model_id ) ?? new DeliveryType();
            ...
            if ($request['cities']){
                $cities = $model->cities_id ? json_decode($model->cities_id) : new \stdClass();
                foreach ($request['cities'] as $key => $data){
                    $cities->$key = $data;
                }
                $model->cities_id = json_encode($cities);
