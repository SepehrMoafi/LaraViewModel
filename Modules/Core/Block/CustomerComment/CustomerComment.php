...
namespace Modules\Core\Block\CustomerComment;

use Modules\Core\Block\BlockInterface;
use Modules\Core\Entities\RouteBlockItem;

class CustomerComment implements BlockInterface
{
    public $routeBlockConfig = [];

    public $title = 'نظر مشتریان';
    ...
    public function renderFront()
    {
        ...
        return view('core::theme_master.blocks.CustomerComment.' . $template, compact('block', 'item_obj', 'block_obj'));
    ...
    public function renderAdmin()
    {
        ...
        $this->routeBlockConfig = $item_obj->configJson;
        $block = $this;
        return view('core::theme_admin.blocks.CustomerComment.CustomerComment', compact('block', 'item_obj'));
    }
        ...
        if (request('config')) {
            ...
            foreach (request('config') as $key => $value) {
                if (str_starts_with($key, 'image')) {
                    $value->image = $this->uploadFile(request(), 'config.' . $key, 'customer_comment');
                }
            }
