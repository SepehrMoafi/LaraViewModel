...
namespace Modules\Core\Block\CardConnectedBlock;

use Modules\Core\Block\BlockInterface;
use Modules\Core\Entities\RouteBlockItem;

class CardConnectedBlock implements BlockInterface
{
    public $routeBlockConfig = [];

    public $title = 'کارت و ایکون';
    ...
    public function renderFront()
    {
        ...
        $block = $this;
        $template = @ $this->routeBlockConfig->template ?? 'place_1';
        return view('core::theme_master.blocks.CardConnectedBlock.' . $template, compact('block', 'item_obj', 'block_obj'));
    ...
    public function renderAdmin()
    {
        ...
        $this->routeBlockConfig = $item_obj->configJson;
        $block = $this;
        return view('core::theme_admin.blocks.CardConnectedBlock.CardConnectedBlock', compact('block', 'item_obj'));
    }
     ...
     * @return void
     */
    public function checkIcon(mixed $config): void
    {
        $icons = ['icon1', 'icon2', 'icon3'];
        $reqConf = request()->config;
        foreach ($icons as $icon) {
            if (isset($reqConf[$icon])) {
                $config->$icon = $this->uploadFile(request(), 'config.' . $icon, 'icon_text');
            }
        }
    }
