...
trait CreateFinanceDoc
{
    public function CreateFinanceDoc ($data , $order){
        try {
            if (@ $order->params_json->sale_voucher->status == 1 ){
                return $order ;
            }
            $url = $this->base_path."/sale/vouchers";
            $headers = array(
            ...
            if ($response) {
                if ($httpcode == '200'){
                    ...
                    $sale_voucher = new \stdClass();
                    $sale_voucher->status = 1 ;
                    $sale_voucher->res = $res;
                    $order_prams->sale_voucher = $sale_voucher;
                    ...
                    $error_log->description = 'در ثبت سند مشکلی پیش آمد';
                    $error_log->reason = 'CreateFinanceDoc';
                ...
                $error_log = new ErrorLog();
                $error_log->description = '  در ثبت سند مشکلی پیش آمد ۲';
                $error_log->reason = 'CreateFinanceDoc';
                $error_log->errorable_id = $order->id;
                $error_log->errorable_type = Order::class;
                $error_log->save();
